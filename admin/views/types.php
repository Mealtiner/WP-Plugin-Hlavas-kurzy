...
<div class="wrap hlavas-terms-wrap">
	...
	<table class="wp-list-table widefat fixed striped">
		<thead>
			<tr>
				...
				<th>Název typu</th>
				<th style="width: 85px;">Kurz</th>
				<th style="width: 85px;">Zkouška</th>
				<th style="width: 110px;">Termínů kurzů</th>
				<th style="width: 120px;">Termínů zkoušek</th>
				<th style="width: 150px;">Akce</th>
			</tr>
		</thead>
		<tbody>
			<?php if ( empty( $types ) ) : ?>
				<tr><td colspan="7">Zatím nejsou založené žádné typy kvalifikací.</td></tr>
			<?php else : ?>
				<?php foreach ( $types as $row ) : ?>
					<tr>
						...
						<td>
							<?php
							$course_counts = (int) ( $row->future_course_terms ?? 0 ) . '/' . (int) ( $row->total_course_terms ?? 0 );
							echo esc_html( $course_counts );
							?>
						</td>
						<td>
							<?php
							$exam_counts = (int) ( $row->future_exam_terms ?? 0 ) . '/' . (int) ( $row->total_exam_terms ?? 0 );
							echo esc_html( $exam_counts );
							?>
						</td>
						<td>
							<a class="button button-small" href="<?php echo esc_url( admin_url( 'admin.php?page=hlavas-terms-types&type_id=' . (int) $row->id . '#hlavas-type-form' ) ); ?>">Upravit</a>
							<a
	...
	<p class="description">Formát u počtů termínů je <strong>budoucí/celkem</strong>.</p>

	<div class="hlavas-type-form-wrap" id="hlavas-type-form">
