...
class Hlavas_Terms_Label_Builder {
	 ...
	 * zkouška: "Zkouška z profesní kvalifikace 75-008-N (22. dubna 2026)"
	 * fallback without qualification code: "zkouška: 22. dubna 2026"
	 *
	 * @param string      $term_type Kurz nebo zkouška.
	 * @param string      $date_start Datum od.
	 * @param string|null $date_end Datum do.
	 * @param string|null $qualification_code Kód kvalifikace pro zkoušku.
	 * @return string
	 */
	public static function build( string $term_type, string $date_start, ?string $date_end = null, ?string $qualification_code = null ): string {
		...
		if ( $date_end && $date_end !== $date_start ) {
			...
			return self::build_range_label( 'kurz', $start, $end );
		}

		return 'kurz: ' . self::format_single_date( $start );
	}
